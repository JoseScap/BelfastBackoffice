...
//   title: 'Reservations | Belfast Backoffice',
//   description: 'Reservation management for Belfast Backoffice',
// };
...
const ReservationsPage = () => {
  ...
  const filteredAppointments = mockAppointments.filter(appointment => {
    const matchesSearch = 
      ...
      appointment.status.value.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  // Sort appointments by check-in date (most recent first)
  ...
  const getStatusColor = (status: AppointmentStatusValue) => {
    switch (status) {
      case AppointmentStatusValue.APPROVED:
        return 'bg-success-500 text-white';
      case AppointmentStatusValue.REQUESTED:
        return 'bg-warning-500 text-white';
      case AppointmentStatusValue.CHECK_IN:
        return 'bg-blue-500 text-white';
      case AppointmentStatusValue.CHECK_OUT:
        return 'bg-gray-500 text-white';
      case AppointmentStatusValue.CANCELLED:
        return 'bg-red-500 text-white';
      case AppointmentStatusValue.OVERBOOKED:
        return 'bg-gray-500 text-white';
      default:
        return 'bg-gray-500 text-white';
    }
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="w-full sm:w-auto">
              ...
              >
                ...
                <option value={AppointmentStatusValue.REQUESTED}>{AppointmentStatusValue.REQUESTED}</option>
                <option value={AppointmentStatusValue.APPROVED}>{AppointmentStatusValue.APPROVED}</option>
                <option value={AppointmentStatusValue.CHECK_IN}>{AppointmentStatusValue.CHECK_IN}</option>
                <option value={AppointmentStatusValue.CHECK_OUT}>{AppointmentStatusValue.CHECK_OUT}</option>
                <option value={AppointmentStatusValue.CANCELLED}>{AppointmentStatusValue.CANCELLED}</option>
                <option value={AppointmentStatusValue.OVERBOOKED}>{AppointmentStatusValue.OVERBOOKED}</option>
              </select>
            </div>
              ...
              <tbody>
                {currentAppointments.map((appointment) => (
                  <tr key={appointment.id}>
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p className={`inline-flex rounded-full py-1 px-3 text-sm font-medium ${getStatusColor(appointment.status.value as AppointmentStatusValue)}`}>
                        {appointment.status.value}
                      </p>
                    </td>
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p className="text-black dark:text-white capitalize">
                        {translateSource(appointment.source)}
                      </p>
                    </td>
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p className="text-black dark:text-white">
